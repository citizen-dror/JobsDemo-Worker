...
namespace JobQueueSystem.WorkerNode.Services
{
    public class WorkerNodeService : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            var registration = await _apiClient.RegisterWorker(worker);
        ...
        private async void SendHeartbeat(object state)
        {
            try
            {
                await _apiClient.SendHeartbeat(_workerId, _status, _activeJobs.Count);
        ...
        public async Task<bool> AcceptJob(Job job)
        {
            ...
            _ = Task.Run(async () =>
            {
                ...
                catch (Exception ex)
                {
                    ...
                    await _apiClient.UpdateJobStatus(job);
                }
                finally
                {
        ...
        private async Task ProcessJob(Job job)
        {
            ...
            try
            {
                ...
                var result = await _jobProcessor.ProcessJob(job, new Progress<int>(async progress =>
                {
                    ...
                    await _apiClient.UpdateJobProgress(job.Id, progress);
                }));
        ...
        public override Task StopAsync(CancellationToken stoppingToken)
        {
            ...
            if (!string.IsNullOrEmpty(_workerId))
            {
                try
                {
                    _apiClient.UpdateWorkerStatus(_workerId, WorkerStatus.Offline).Wait();
                }
                catch (Exception ex)
                {
